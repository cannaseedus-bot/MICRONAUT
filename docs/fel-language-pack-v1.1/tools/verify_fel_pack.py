...
# Expected hashes for the GOLDEN VECTOR (v1.1)
...
# -----------------------------
# Legality / verifier rules (FEL v1.1)
# -----------------------------

def assert_invariants(events):
    seen_attestations = set()
    attestation_prev = {}
    attestation_abi = None
    ...
    for i, event in enumerate(events):
        ...
        if event.get("v") not in {"fel.v1", "fel.v1.1"}:
            die(f"event[{i}] v must be fel.v1 or fel.v1.1")
        ...
        # L2 type↔fold constraints (v1.1)
        if event_type == "set" and fold != "⟁STATE_FOLD⟁":
            die(f"event[{i}] set must target ⟁STATE_FOLD⟁")
        if event_type == "delta" and fold != "⟁STATE_FOLD⟁":
            die(f"event[{i}] delta must target ⟁STATE_FOLD⟁")
        ...
        if event_type == "delta":
            ...
            if not isinstance(patch, dict):
                die(f"event[{i}] delta requires patch object")
            op = patch.get("op")
            path = patch.get("path")
            if op not in {"add", "replace", "remove"}:
                die(f"event[{i}] patch.op must be add|replace|remove")
            if not isinstance(path, str) or not path.startswith("/"):
                die(f"event[{i}] patch.path must start with '/'")
            if op == "remove" and "value" in patch:
                die(f"event[{i}] remove patch must not include value")
            if op in {"add", "replace"} and "value" not in patch:
                die(f"event[{i}] add/replace requires value")

        # Seal rules
        if event_type == "seal":
            if event.get("lane") != FOLD_TO_LANE["⟁STORAGE_FOLD⟁"]:
                die(f"event[{i}] seal lane must match storage lane law")
            source = event.get("source")
            if not isinstance(source, dict):
                die(f"event[{i}] seal requires source object")
            source_tick = source.get("tick")
            if not isinstance(source_tick, int) or source_tick > event["tick"]:
                die(f"event[{i}] seal source.tick must be <= seal tick")

        # Attestation rules
        if event_type == "attest":
            policy_hash = event.get("policy_hash")
            abi_hash = event.get("abi_hash")
            if attestation_policy is None:
                attestation_policy = policy_hash
            elif policy_hash != attestation_policy:
                die(f"event[{i}] policy_hash drift not allowed")
            if attestation_abi is None:
                attestation_abi = abi_hash
            elif abi_hash != attestation_abi:
                die(f"event[{i}] abi_hash drift not allowed")

            line_hash = sha256_hex(canon_fel_line(event).encode("utf-8"))
            prev = event.get("prev")
            if prev is not None and prev not in seen_attestations:
                die(f"event[{i}] prev must reference prior attestation")
            if line_hash in seen_attestations:
                die(f"event[{i}] duplicate attestation hash")
            seen_attestations.add(line_hash)
            attestation_prev[line_hash] = prev
